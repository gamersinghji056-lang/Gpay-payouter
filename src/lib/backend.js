...
function stateFromRows(fallback, settings, providers, entries, deposits, qrs, withdrawals, merchantSettlements, shareLinks, upiAccounts = [], charges = []) {
  ...
  next.deposits = (deposits || []).map(row => ({ id: row.id, userId: ids.get(row.provider_id) || row.provider_id,
    ...
    address: row.destination_address, status: row.status, txHash: row.tx_hash || '', createdAt: row.created_at, expiresAt: row.expires_at || '', confirmedAt: row.confirmed_at || '', source: row.source }));
...
async function loadState(fallback) {
  ...
  const [settings, providers, entries, deposits, qrs, withdrawals, merchantSettlements, shareLinks, upiAccounts, charges] = await Promise.all([
    ...
    supabase.from('deposit_requests').select('id,provider_id,requested_usdt,expected_usdt,rate,inr_value,destination_address,status,tx_hash,created_at,expires_at,confirmed_at,source').order('created_at', { ascending: false }).limit(500),
    ...
    supabase.from('provider_upi_accounts').select('id,provider_id,label,upi_id,mobile,apk_mobile,gpay_login_id,qr_data,status,merchant_operational,blocked_by_user,blocked_by_admin,admin_block_reason,archived_at,configured_limit_inr,allocated_limit_inr,bank_name,bank_account_number,account_holder_name,ifsc_code,bank_branch,account_note'),
...
async function updateUpiLifecycle(accountId, action, reason) { return callFunction('provider-write', { action, upi_account_id: accountId, reason }); }
...
async function portalLogin(role, loginId, password) { return callPublicFunction('portal-auth', { action: 'login', role, login_id: loginId, password }); }
async function portalLogout(token) { return callPublicFunction('portal-auth', { action: 'logout', session_token: token }); }
async function portalResolve(role, token) { return callPublicFunction('portal-resolve', { role, session_token: token }); }
