...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [settings, ledger, deposits, qrs, settlements, withdrawals, upiAccounts, charges, portalAccounts] = await Promise.all([
      ...
      ids.length ? admin.from("ledger_entries").select("id,provider_id,upi_account_id,entry_type,credit_rate,amount_inr,amount_usdt,rate,merchant_commission_rate,merchant_commission_inr,bank_name,account_number,transaction_date,note,status,created_at,updated_at,idempotency_key,is_voided,voided_at,void_reason,edited_at,edit_reason").in("provider_id", ids).order("transaction_date", { ascending: false }).limit(acct.role === "user" ? 250 : 500) : Promise.resolve({ data: [], error: null }),
      acct.role === "user" && ids.length ? admin.from("deposit_requests").select("id,provider_id,requested_usdt,expected_usdt,rate,inr_value,destination_address,status,tx_hash,created_at,expires_at,confirmed_at,source").in("provider_id", ids).order("created_at", { ascending: false }).limit(250) : Promise.resolve({ data: [], error: null }),
    ...
    for (const p of providers || []) {
      ...
      user.upiAccounts = (upiAccounts.data || []).filter((a) => a.provider_id === p.id).map((a) => { const v: any = ua.get(a.id) || {}; return { id: a.id, label: a.label, upi: a.upi_id || "", mobile: a.mobile || "", apk: a.apk_mobile || "", gpayLogin: a.gpay_login_id || "", qrData: a.qr_data || "", status: a.status, merchantOperational: a.merchant_operational, blockedByUser: a.blocked_by_user === true, blockedByAdmin: a.blocked_by_admin === true, adminBlockReason: a.admin_block_reason || "", archivedAt: a.archived_at || "", configuredLimit: Number(a.configured_limit_inr || 0), allocatedLimit: Number(a.allocated_limit_inr || 0), bankName: a.bank_name || "", bankAccountNumber: a.bank_account_number || "", accountHolderName: a.account_holder_name || "", ifscCode: a.ifsc_code || "", bankBranch: a.bank_branch || "", accountNote: a.account_note || "", accounting: v, collection: Number(v.total_collection_inr || 0), availableLimit: Number(v.available_limit_inr || 0), qrs: (qrs.data || []).filter((q) => q.upi_account_id === a.id).map((q) => ({ id: q.id, name: q.display_name || "QR", storagePath: q.storage_path, data: sq.get(q.id) || "" })) }; });
