...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.action === "reveal" && body.public_token && body.upi_account_id) { const { data, error } = await admin.rpc("reveal_upi_gpay_password_by_share", { p_token: body.public_token, p_upi_account_id: body.upi_account_id, p_encryption_key: key }); if (error) throw error; return corsJson({ password: data }); }
