...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [ledger, deposits, qrs, merchantSettlements, withdrawals, upiAccounts, charges] = await Promise.all([
      ...
      link.scope === "merchant" ? admin.from("merchant_settlements").select("*") : Promise.resolve({ data: [], error: null }),
      link.scope === "merchant" ? admin.from("withdrawal_requests").select("*").eq("requester_type", "merchant").in("status", ["pending", "paid"]) : link.scope === "user" ? admin.from("withdrawal_requests").select("*").eq("requester_type", "provider").eq("provider_id", link.provider_id).in("status", ["pending", "paid"]) : Promise.resolve({ data: [], error: null }),
      ...
      link.scope === "merchant" ? admin.from("merchant_charges").select("id,provider_id,upi_account_id,amount_inr,user_name,upi_id,mobile,charge_date,reference,note,status,created_at").eq("status", "active") : Promise.resolve({ data: [], error: null }),
    ]);
    ...
    for (const provider of providers || []) {
      ...
      for (const account of user.upiAccounts) { const { data: upiAccounting } = await admin.rpc("accounting_for_upi", { p_upi_account_id: account.id }); const value = upiAccounting?.[0]; if (value) { account.accounting = value; account.collection = Number(value.total_collection_inr || 0); account.availableLimit = Number(value.available_limit_inr || 0); } account.qrs = []; for (const qr of (qrs.data || []).filter(row => row.upi_account_id === account.id)) { const { data: signed } = await admin.storage.from("provider-qr").createSignedUrl(qr.storage_path, 300); account.qrs.push({ id: qr.id, name: qr.display_name || "QR", storagePath: qr.storage_path, data: signed?.signedUrl || "" }); } }
      if (privateView) { user.fundingMode = provider.funding_model; user.depositAddress = provider.unique_deposit_address || ""; }
