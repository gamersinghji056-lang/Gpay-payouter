...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [ledger, deposits, qrs, merchantSettlements, withdrawals] = await Promise.all([
    ...
    if (ledger.error || deposits.error || qrs.error || merchantSettlements.error || withdrawals.error) throw ledger.error || deposits.error || qrs.error || merchantSettlements.error || withdrawals.error;
    ...
    for (const provider of providers || []) {
      ...
      const user = { id: provider.user_code, remoteId: provider.id, name: provider.name, telegram: provider.telegram_username || "", upi: provider.upi_id || "", mobile: provider.mobile || "", apk: provider.apk_mobile || "", gpayLogin: provider.gpay_login_id || "", qrs: [], token: privateView ? token : "", active: provider.status !== "deleted", status: provider.status, pauseReason: provider.pause_reason || "", accounting: { collection_inr: accounting.collection_inr || 0, collection_capacity_inr: accounting.collection_capacity_inr || 0, frozen_inr: 0, successful_withdrawal_inr: 0, user_usdt_inr: 0, merchant_settled_inr: 0, confirmed_deposit_inr: privateView ? accounting.confirmed_deposit_inr || 0 : 0, commission_earned_inr: 0 } };
