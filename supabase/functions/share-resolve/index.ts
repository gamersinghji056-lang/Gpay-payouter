...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [ledger, deposits, qrs, merchantSettlements, withdrawals, upiAccounts, charges] = await Promise.all([
      ...
      link.scope === "merchant" ? admin.from("withdrawal_requests").select("*").eq("requester_type", "merchant").in("status", ["pending", "paid"]) : Promise.resolve({ data: [], error: null }),
