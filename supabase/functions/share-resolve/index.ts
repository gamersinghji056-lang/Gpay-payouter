...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const provider of providers || []) {
      ...
      for (const account of user.upiAccounts) { const { data: upiAccounting } = await admin.rpc("accounting_for_upi", { p_upi_account_id: account.id }); const value = upiAccounting?.[0]; if (value) { account.accounting = value; account.collection = Number(value.total_collection_inr || 0); account.availableLimit = Number(value.available_limit_inr || 0); } }
